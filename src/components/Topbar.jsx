  ...
  return (
    <div className="flex-1 flex flex-wrap items-center justify-between gap-4">
      <div>
        ...
        <p className="text-slate-500 mt-1">نظرة عامة على عمليات الميناء</p>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <button className="relative bg-white w-14 h-14 rounded-2xl shadow-soft border border-slate-200 flex items-center justify-center text-slate-700">
          <Bell size={20} />
          {notificationCount > 0 && (
            <span className="absolute -top-1 -left-1 bg-red-500 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center">{notificationCount}</span>
          )}
        </button>
        ...
        <div className="bg-white rounded-2xl px-4 py-3 shadow-soft border border-slate-200 flex items-center gap-3 min-w-[220px]">
          ...
          <div>
            <div className="font-bold text-slate-900">{user?.first_name || user?.username || 'المستخدم'}</div>
